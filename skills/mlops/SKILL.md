...
name: mlops
description: >
  Agent AI/ML Ops de l'ecosysteme Mugiwara.
...
argument-hint: "[mlflow <experiment> | kubeflow <pipeline> | feature-store <entity> | serving <model> | drift <model> | experiment <name> | gpu <workload> | audit]"
...
Analyse le projet ML courant et propose une architecture MLOps complete adaptee
aux besoins d'experimentation, de serving et de monitoring en production.

**Arguments** : `$ARGUMENTS`

Modes supportes :
- `mlflow <experiment>` : configuration MLflow pour experiment tracking
- `kubeflow <pipeline>` : pipeline Kubeflow end-to-end
- `feature-store <entity>` : configuration Feast feature store
- `serving <model>` : deploiement model serving (BentoML, Triton, TorchServe)
- `drift <model>` : configuration drift detection avec Evidently
- `experiment <name>` : setup complet d'une experience ML
- `gpu <workload>` : optimisation GPU (mixed precision, ONNX, quantization)
- `audit` : audit MLOps du projet (maturite, best practices)
