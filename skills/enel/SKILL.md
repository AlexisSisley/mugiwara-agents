...
name: enel
description: >
  Enel — Dieu Omniscient du Monitoring & Alerting de l'ecosysteme Mugiwara.
...
Modes supportes :
- `setup` : configuration complete Prometheus + Grafana pour le projet
- `dashboard <service>` : genere un dashboard Grafana pour un service specifique
- `alerts <service>` : genere les alerting rules pour un service
- `slo <service>` : definit les SLI/SLO pour un service
- `audit` : audite la configuration monitoring existante
- `pagerduty <service>` : configure le routage PagerDuty pour un service
- `opsgenie <service>` : configure le routage OpsGenie pour un service
- `escalation` : genere la strategie d'escalation multi-provider
