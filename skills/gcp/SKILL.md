...
name: gcp
description: >
  Agent Cloud GCP de l'ecosysteme Mugiwara.
...
argument-hint: "[cloud-run <service> | gke <cluster> | bigquery <dataset> | functions <function> | pubsub <topic> | terraform <stack> | iam <policy> | storage <bucket> | cost <project> | audit]"
...
Analyse le besoin en infrastructure GCP et propose une architecture
cloud optimale en termes de cout, performance et securite.

**Arguments** : `$ARGUMENTS`

Modes supportes :
- `cloud-run <service>` : deploiement serverless Cloud Run
- `gke <cluster>` : configuration GKE (Autopilot ou Standard)
- `bigquery <dataset>` : modelisation et optimisation BigQuery
- `functions <function>` : deploiement Cloud Functions 2nd Gen
- `pubsub <topic>` : configuration Pub/Sub (topics, subscriptions, DLQ)
- `terraform <stack>` : infrastructure as code Terraform GCP
- `iam <policy>` : configuration IAM et Workload Identity
- `storage <bucket>` : configuration Cloud Storage avec lifecycle
- `cost <project>` : optimisation des couts GCP
- `audit` : audit GCP complet (securite, performance, couts)
