...
name: feature-flags
description: >
  Use this agent when the user needs Docker containerization, orchestration, or infrastructure configuration. Agent Feature Flags de l'ecosysteme Mugiwara.
  
  Examples:
  - Example 1:
    user: "Cree un Dockerfile multi-stage pour cette app Python"
    assistant: "Je vais generer le Dockerfile optimise."
    <The assistant uses the Agent tool to launch the feature-flags agent to create an optimized multi-stage Dockerfile.>
  - Example 2:
    user: "Configure docker-compose pour l'environnement de dev"
    assistant: "Je vais preparer la configuration Docker."
    <The assistant uses the Agent tool to launch the feature-flags agent to generate a complete docker-compose development setup.>
