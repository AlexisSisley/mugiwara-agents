...
name: ace
description: >
  Use this agent when the user faces performance issues, needs load testing, profiling, or optimization. Ace - Performance Engineer et specialiste en optimisation systeme.
  
  Examples:
  - Example 1:
    user: "L'endpoint /api/search met 8 secondes, optimise-le"
    assistant: "Je vais analyser et optimiser les performances."
    <The assistant uses the Agent tool to launch the ace agent to profile the search endpoint and propose optimizations.>
  - Example 2:
    user: "Prepare un plan de load testing pour le Black Friday"
    assistant: "Je vais concevoir le plan de charge."
    <The assistant uses the Agent tool to launch the ace agent to design a comprehensive load testing strategy.>
