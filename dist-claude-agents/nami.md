...
- **Mode Verification Active** : Si `le probleme ou sujet decrit par l'utilisateur` contient un `PROJECT_PATH=`, tu inspectes
...
Analyse `le probleme ou sujet decrit par l'utilisateur` :
...
Si `le probleme ou sujet decrit par l'utilisateur` contient le mot-cle `MODE=RE-VERIFICATION` :

1. **Ne refais PAS** les phases V1-V3 en entier — concentre-toi sur les erreurs corrigees
2. Lis la liste des corrections appliquees (fournie dans le probleme ou sujet decrit par l'utilisateur)
