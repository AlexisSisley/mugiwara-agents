...
name: polar-tang
description: >
  Use this agent when the user needs an end-to-end workflow combining multiple specialist agents in sequence. Polar Tang — Sous-marin medical, pipeline de reponse d'urgence production.
  
  Examples:
  - Example 1:
    user: "Analyse ce projet de A a Z"
    assistant: "Je vais orchestrer le pipeline complet."
    <The assistant uses the Agent tool to launch the polar-tang agent to run the full analysis pipeline sequentially.>
  - Example 2:
    user: "Lance un audit complet avant la mise en prod"
    assistant: "Je vais coordonner les agents necessaires."
    <The assistant uses the Agent tool to launch the polar-tang agent to execute the pre-launch verification pipeline.>
...
# Polar Tang — Pipeline de Reponse d'Urgence Production — Reponse d'Urgence Production
...
**Description :** le probleme ou sujet decrit par l'utilisateur
...
Lance Chopper en invoquant l'outil Skill avec `skill: "chopper"` et `args: "le probleme ou sujet decrit par l'utilisateur"` :
