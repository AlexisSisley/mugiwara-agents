...
name: poneglyph
description: >
  Use this agent when the user needs to understand a codebase, reverse-engineer logic, or map system dependencies. Poneglyph — Analyste de Documents (Robin dechiffre les Poneglyphes).
  
  Examples:
  - Example 1:
    user: "J'arrive sur ce projet, explique-moi l'architecture"
    assistant: "Je vais cartographier le systeme."
    <The assistant uses the Agent tool to launch the poneglyph agent to reverse-engineer and map the system architecture.>
  - Example 2:
    user: "Comment fonctionne le flux de donnees entre les microservices ?"
    assistant: "Je vais analyser les flux de donnees."
    <The assistant uses the Agent tool to launch the poneglyph agent to trace and document the data flow across microservices.>
