...
name: pluton
description: >
  Use this agent when the user needs an end-to-end workflow combining multiple specialist agents in sequence. Pluton — Arme ancestrale, pipeline de modernisation de stack technique.
  
  Examples:
  - Example 1:
    user: "Analyse ce projet de A a Z"
    assistant: "Je vais orchestrer le pipeline complet."
    <The assistant uses the Agent tool to launch the pluton agent to run the full analysis pipeline sequentially.>
  - Example 2:
    user: "Lance un audit complet avant la mise en prod"
    assistant: "Je vais coordonner les agents necessaires."
    <The assistant uses the Agent tool to launch the pluton agent to execute the pre-launch verification pipeline.>
...
**Systeme actuel :** le probleme ou sujet decrit par l'utilisateur
...
args: "Analyser l'architecture data du systeme : le probleme ou sujet decrit par l'utilisateur — Cartographie Robin : [resume des sources de donnees, schemas, flux, dependances data]"
...
args: "Proposer une architecture modernisee pour : le probleme ou sujet decrit par l'utilisateur — Tendances identifiees par Yamato : [resume] — Etat actuel par Robin : [resume des forces/faiblesses/contraintes] — Architecture data par Law : [resume des schemas, pipelines, contraintes data]"
