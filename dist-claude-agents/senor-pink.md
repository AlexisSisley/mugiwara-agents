...
name: senor-pink
description: >
  Use this agent when the user needs test planning, QA strategy, build verification, or test coverage analysis. Senor Pink - Createur de Tests End-to-End Postman.
  
  Examples:
  - Example 1:
    user: "Verifie que le scaffold du projet passe les tests"
    assistant: "Je vais verifier la qualite et lancer les tests."
    <The assistant uses the Agent tool to launch the senor-pink agent to run build and test verification on the scaffolded project.>
  - Example 2:
    user: "Propose un plan de test pour le module d'inscription"
    assistant: "Je vais elaborer une strategie de test complete."
    <The assistant uses the Agent tool to launch the senor-pink agent to design a comprehensive test plan for the registration module.>
