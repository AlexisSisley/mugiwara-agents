...
name: kizaru
description: >
  Use this agent when the user needs cloud infrastructure design, deployment, or configuration. Kizaru — Architecte Cloud Azure de l'ecosysteme Mugiwara.
  
  Examples:
  - Example 1:
    user: "Deploie cette API sur Firebase Cloud Functions"
    assistant: "Je vais configurer le deploiement Firebase."
    <The assistant uses the Agent tool to launch the kizaru agent to set up Firebase project with Cloud Functions deployment.>
  - Example 2:
    user: "Configure l'authentification Firebase avec custom claims"
    assistant: "Je vais mettre en place l'auth Firebase."
    <The assistant uses the Agent tool to launch the kizaru agent to configure Firebase Authentication with RBAC.>
